...
const Genre = () => {
  ...
  const fetchGenreAndTracks = async () => {
    setLoading(true);
    setError(null);
    try {
  ...
  const handleTrackClick = (track, index) => {
    // Pass the genre source when playing tracks
    const source = `/genre/${genre}`;
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-neutral-400 mt-2">
              The genre you're looking for doesn't exist. Please check the URL and try again.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold capitalize">{genre} Music</h1>
          {genreData?.description && (
            <p className="text-neutral-400 mt-2">{genreData.description}</p>
          )}
        </div>
      </div>
